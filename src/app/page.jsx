...
function FeedPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/posts?${params}`).then((response) => response.json()),
      ...
      .then(([postData, userData]) => {
        ...
        setShowOnboarding(Boolean(userData.user && userData.user.role === "INFLUENCER" && !userData.user.isProfileCompleted))
      })
    ...
    <main id="main-content" className="h-[100dvh] w-full overflow-hidden bg-background">
      ...
      <div className="mx-auto grid h-full w-full max-w-[1480px] grid-cols-1 gap-5 overflow-hidden px-3 pb-[76px] pt-[76px] sm:px-5 md:pb-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,680px)_300px] xl:grid-cols-[280px_minmax(0,680px)_320px] xl:gap-6">
        ...
        <section ref={composerRef} className="custom-scrollbar min-h-0 min-w-0 overflow-y-auto pb-8 lg:col-start-2">
          <div className="mb-4 flex items-center justify-between px-1">
            <div>
              <h1 className="text-xl font-bold tracking-[-0.025em] sm:text-2xl">
                {filter ? filter.text : "Bảng tin cộng đồng"}
              </h1>
              <p className="mt-1 text-xs text-muted-foreground">
                Mẻ câu, kỹ thuật và câu chuyện mới nhất từ mặt nước.
              </p>
            </div>
            {filter && (
              <Link href="/" aria-label="Xóa bộ lọc" className="kinetic flex h-9 w-9 items-center justify-center rounded-full bg-muted text-muted-foreground hover:bg-primary/15 hover:text-primary">
          ...
          {nextCursor && (
            <button type="button" disabled={loadingMore} onClick={loadMore} className="kinetic mx-auto flex rounded-lg bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground hover:bg-primary/90 active:scale-[0.98] disabled:opacity-50">
              {loadingMore ? "Đang tải..." : "Xem thêm nhật ký"}
            </button>
          )}
        </section>
